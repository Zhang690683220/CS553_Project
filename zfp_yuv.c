...
int main(int argc, char** argv){
    ...
    split_yuv444("./image/photo/photo.yuv", width, height, y_buffer, u_buffer, v_buffer);
    const int range = 256;
    ...
    sprintf(zfpYFileName, "./image/photo/Y_%s.zfp",argv[1]);
    sprintf(zfpUFileName, "./image/photo/U_%s.zfp",argv[1]);
    sprintf(zfpVFileName, "./image/photo/V_%s.zfp",argv[1]);
    /* Compression */
    compress(zfpYFileName, y_buffer, height, width, tolerance, 0);
    compress(zfpUFileName, u_buffer, height, width, tolerance, 0);
    compress(zfpVFileName, v_buffer, height, width, tolerance, 0);

    /* Decompression */
    compress(zfpYFileName, y_buffer, height, width, tolerance, 1);
    compress(zfpUFileName, u_buffer, height, width, tolerance, 1);
    compress(zfpVFileName, v_buffer, height, width, tolerance, 1);

    char outputYUVName[128];
    sprintf(outputYUVName, "./image/photo/zfp_%s.yuv",argv[1]);
