...
int main(int argc, char** argv){
    ...
    {
        char inputRGBName[128];
        sprintf(inputRGBName, "./video/rgb/frames%d.rgb", i);
        

        split_rgb24(inputRGBName, width, height, r_buffer, g_buffer, b_buffer);
        
        char zfpRFileName[128];
        char zfpGFileName[128];
        char zfpBFileName[128];
        sprintf(zfpRFileName, "./video/rgb/R_%sframes%d.zfp",argv[1], i);
        sprintf(zfpGFileName, "./video/rgb/G_%sframes%d.zfp",argv[1], i);
        sprintf(zfpBFileName, "./video/rgb/B_%sframes%d.zfp",argv[1], i);

        compress(zfpRFileName, r_buffer, height, width, rate, 0);
        compress(zfpGFileName, g_buffer, height, width, rate, 0);
        compress(zfpBFileName, b_buffer, height, width, rate, 0);

    
        compress(zfpRFileName, r_buffer, height, width, rate, 1);
        compress(zfpGFileName, g_buffer, height, width, rate, 1);
        compress(zfpBFileName, b_buffer, height, width, rate, 1);

        char outputRGBName[128];
        sprintf(outputRGBName, "./video/rgb/zfp_%s_frame%d.rgb",argv[1], i);
